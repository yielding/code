...
static void node_free(mrb_state* mrb, void* p) { delete static_cast<node*>(p); }
static const mrb_data_type node_type = { "Node", node_free };

// 2. 클래스 정의 + 인스턴스 타입 전환
auto cls = mrb_define_class(mrb, "Node", mrb->object_class);
MRB_SET_INSTANCE_TT(cls, MRB_TT_DATA);      // ← 빠뜨리면 DATA_PTR가 없다

// 3. initialize — 재초기화 안전 순서
auto ni_initialize(mrb_state* mrb, mrb_value self) -> mrb_value
{
  mrb_value path;
  mrb_get_args(mrb, "S", &path);            // String 타입 체크 포함

  auto p = static_cast<node*>(DATA_PTR(self));
  if (p) node_free(mrb, p);                 // 재호출되면 이전 것 해제
  DATA_PTR(self)  = nullptr;                // ← new 중 GC가 봐도 안전하게
  DATA_TYPE(self) = nullptr;
  DATA_PTR(self)  = new node(RSTRING_PTR(path));
  DATA_TYPE(self) = &node_type;
  return self;
}

// 4. 메서드 — unwrap 후 위임
auto ni_size(mrb_state* mrb, mrb_value self) -> mrb_value
{
  auto f = static_cast<node*>(mrb_data_get_ptr(mrb, self, &node_type));
...
```cpp
// node_ext.cpp — C++ 클래스 node를 MD::Node로 노출
auto init_node(mrb_state* mrb) -> void
{
  mrubybind::klass<node>::define(mrb, "MD", "Node")   // module, class
    .ctor<string>()                       // MD::Node.new(path) — "S" 체크
    .method<&node::name>("name")
    .method<&node::size>("size")
    .method<&node::deleted>("deleted?")
    .method<&node::seek>("seek")          // 인자 지정자는 시그니처에서 유도
    .method<&node::read>("read")          // vector<uint8_t> → binary String
    .method<&node::save_to>("save_to");
}
```

- `define(mrb, "Node")` — 최상위 클래스.  `define(mrb, "MD", "Node")`
...
- `ctor<Args...>()` — `initialize`를 바인딩.  인자 타입 목록에서
  ...
  `MD::Node.new(123)`은 crash가 아니라 TypeError다.  재초기화 안전
...
auto main(int argc, const char* argv[]) -> int
{
  ...
  init_node_system(mrb);    // MD::Nodesystem
  init_node(mrb);           // MD::Node
...
f = MD::Node.new("/evidence/img.dd")   # 스크립트 생성 (GC-owned)
puts f.size
f.save_to("/tmp/out")
```
...
1. **도메인 클래스는 전용 모듈 아래 정의한다.**  `mrubybind`는
   `define(mrb, "MD", "Node")` 오버로드로 이를 지원하며, 모듈 아래
   이름은 내장 최상위 클래스와 절대 충돌하지 않는다.
2. **이름 자체도 내장과 겹치지 않게 고른다.**  md.analyzer의 도메인
   클래스는 처음 `MD::File`이었다가 `MD::Node`로 개명했다 — 포렌식
   관점에서 파일시스템 안의 엔트리는 file이 아니라 **node**이고
   (디렉토리·심링크·삭제 흔적도 모두 노드다), 덤으로 내장 `File`과의
   혼동 여지도 사라진다.

```ruby
f = MD::Node.new("/evidence/img.dd")   # 도메인 객체
