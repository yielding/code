...
private:
  ...
  struct Node 
  {
    ...
    std::pair<const Key, Value> _value; 
  ...
  //
  auto find_node(const Key& key) const -> std::pair<Node*, Node*>;
  ...
  // tree.
  //
  void rebalance_from(Node* where);
  ...
  //
  static Node* cloneTree(Node* to_clone, Node* parent);
...
public:
  ...
  auto operator++(int) -> const DerivedType
  {
...
protected:
  ...
  IteratorBase(const AVLTree* owner = nullptr, Node* curr = nullptr) 
...
public:
  ...
  explicit const_iterator(iterator itr) 
    : IteratorBase<const_iterator,
                 ...
                 const std::pair<const Key, Value>&>(itr._owner, itr._curr) 
  {}
...
AVLTree<Key, Value, Comparator>::Node::Node(const Key& key,
                                            const Value& value,
                                            int height) 
  : _value(key, value), _height(height) 
{}
...
template <typename Key, typename Value, typename Comparator>
AVLTree<Key, Value, Comparator>::AVLTree(Comparator comp) : _comp(comp) 
{
  // Initially, the list of elements is empty and the tree is nullptr.
...
AVLTree<Key, Value, Comparator>::insert(const Key& key, const Value& value) 
{
  ...
  Node* to_insert = new Node(key, value, 1);
  ...
  *curr = to_insert;
  ...
  return std::make_pair(iterator(this, to_insert), true);
}
...
//
template <typename Key, typename Value, typename Comparator>
void AVLTree<Key, Value, Comparator>::rotate_up(Node* node) 
{
  ...
  if (parent->_parent) 
  {
    const int parentSide = (parent != parent->_parent->_children[0]);
    parent->_parent->_children[parentSide] = node;
  } 
  else
  {
    _root = node;
  ...
  parent->_height = 1 + std::max(height(parent->_children[0]), 
                                 height(parent->_children[1]));
  node->_height = 1 + std::max(height(node->_children[0]), 
                               height(node->_children[1]));
}
...
template <typename Key, typename Value, typename Comparator>
int AVLTree<Key, Value, Comparator>::balance_factor(const Node* node) 
{
...
//
template <typename Key, typename Value, typename Comparator>
void AVLTree<Key, Value, Comparator>::rebalance_from(Node* where) 
{
  ...
  while (where != nullptr) 
  {
    ...
    if (const int balance = balance_factor(where); balance == 2 || balance == -2)
    {
      ...
      auto tallChild = where->_children[balance == -2];
      ...
      // its parent.
      if (const int child_balance = balance_factor(tallChild);
      ...
      { 
        ...
        //
        auto tallGrandchild = tallChild->_children[child_balance == -1];

        // Rotate this node up twice.
        rotate_up(tallGrandchild);
        rotate_up(tallGrandchild);

        // Again, pick up the search from this point.
        where = tallGrandchild->_parent;
      }
    }
...
AVLTree<Key, Value, Comparator>::find(const Key& key) 
{
  ...
  // function.
  auto itr = static_cast<const AVLTree*>(this)->find(key);

  // Strip off the constness by wrapping it up as a raw iterator. 
  return iterator(itr._owner, itr._curr);
}
...
AVLTree<Key, Value, Comparator>::find_node(const Key& key) const 
{
  ...
  while (curr != nullptr) 
  {
    ...
    //
      ...
      return std::make_pair(curr, curr->_parent);
  }
  ...
  // gives a type error.
  //
  return std::make_pair(static_cast<Node*>(nullptr), prev);
}
...
template <typename Key, typename Value, typename Comparator>
void AVLTree<Key, Value, Comparator>::splice_out(Node* node) 
{
  ...
  //
  const size_t childIndex = (node->_children[1] != nullptr);
  auto child = node->_children[childIndex];

  // Make sure the other is nullptr. 
  assert(node->_children[!childIndex] == nullptr);
...
AVLTree<Key, Value, Comparator>::erase(iterator where) 
{
  ...
  else 
  {
    ...
    rebalance_from(node == successorParent? successor : successorParent);
  }
...
template <typename Key, typename Value, typename Comparator>
typename AVLTree<Key, Value, Comparator>::Node*
AVLTree<Key, Value, Comparator>::cloneTree(Node* to_clone, Node* parent) 
{
  ...
  auto result = new Node(to_clone->_value.first, to_clone->_value.second, to_clone->_height);

  // Recursively clone the subtrees.
  for (int i = 0; i < 2; ++i)
    result->_children[i] = cloneTree(to_clone->_children[i], result);
...
template <typename Key, typename Value, typename Comparator>
typename AVLTree<Key, Value, Comparator>::Node*
AVLTree<Key, Value, Comparator>::rethread_linked_list(Node* root, Node* predecessor) 
{
...
AVLTree<Key, Value, Comparator>::equal_range(const Key& key) 
{
  ...
  return std::make_pair(iterator(result.first._owner,  result.first._curr),
