////////////////////////////////////////////////////////////////////////////////
//
// Implementation of Bruce Schneier's BLOWFISH algorithm from 
...
namespace 
{
  ...
  inline void bytes_to_block(unsigned char const* p, sblock& b)
  {
    unsigned int y;
    // Left
    b.m_uil = 0;
  ...
  inline void block_to_bytes(sblock const& b, unsigned char* p)
  {
    // Right
    unsigned int y = b.m_uir;
    *--p = Byte(y); y = b.m_uir >> 8;
    *--p = Byte(y); y = b.m_uir >> 16;
    *--p = Byte(y); y = b.m_uir >> 24;
    *--p = Byte(y);

    // Left
    y = b.m_uil;
    *--p = Byte(y); y = b.m_uil >> 8;
    *--p = Byte(y); y = b.m_uil >> 16;
    *--p = Byte(y); y = b.m_uil >> 24;
    *--p = Byte(y);
  }
}
...
////////////////////////////////////////////////////////////////////////////////
//
// Initialization with a fixed string which consists of the hexadecimal 
// digits of PI (less the initial 3) P-array, 18 32-bit subkeys
//
////////////////////////////////////////////////////////////////////////////////
...
BlowFish::BlowFish(unsigned char* ucKey, size_t keysize, const sblock& roChain) 
  : m_oChain0(roChain), m_oChain(roChain)
{
  if (keysize < 1)
    throw BlowFishException("Incorrect key length");
  ...
  for (unsigned i=0; i<18; )
    encrypt_internal(block), m_auiP[i++] = block.m_uil, m_auiP[i++] = block.m_uir;

  for (unsigned j=0; j<4; j++)
    for (int k=0; k<256; )
      encrypt_internal(block), m_auiS[j][k++] = block.m_uil, m_auiS[j][k++] = block.m_uir;
}

// Sixteen Round Encipher of Block
void BlowFish::encrypt_internal(sblock& block)
{
...
void BlowFish::decrypt_internal(sblock& block)
{
  unsigned int uiLeft  = block.m_uil;
  unsigned int uiRight = block.m_uir;
...
void BlowFish::encrypt_inplace(unsigned char* buf, size_t n, int mode)
{
  // Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n%8 != 0))
    throw BlowFishException("Incorrect buffer length");

  sblock work;
  if (mode == CBC) //CBC mode, using the Chain
  {
    sblock chain(m_oChain);
    for (; n >= 8; n -= 8)
    {
      bytes_to_block(buf, work);
      work ^= chain;
      encrypt_internal(work);
      chain = work;
      block_to_bytes(work, buf+=8);
    }
  }
  else if (mode == CFB) //CFB mode, using the Chain
  {
    sblock chain(m_oChain);
    for (; n >= 8; n -= 8)
    {
      encrypt_internal(chain);
      bytes_to_block(buf, work);
      work ^= chain;
      chain = work;
      block_to_bytes(work, buf+=8);
...
// returns false if n is multiple of 8
void BlowFish::decrypt_inplace(unsigned char* buf, size_t n, int mode)
{
  //Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n%8 != 0))
    throw BlowFishException("Incorrect buffer length");
  ...
  if (mode == CBC) //CBC mode, using the Chain
  {
    ...
    for (; n >= 8; n -= 8)
    {
      bytes_to_block(buf, work);
      crypt = work;
      decrypt_internal(work);
      work ^= chain;
      chain = crypt;
      block_to_bytes(work, buf+=8);
    }
  }
  else if (mode == CFB) // CFB mode, using the Chain, not using decrypt()
  {
    sblock crypt, chain(m_oChain);
    for (; n >= 8; n -= 8)
    {
...
// Returns false if n is multiple of 8
void BlowFish::encrypt(const unsigned char* in, unsigned char* out, size_t n, int mode)
{
  // Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n%8 != 0))
    throw BlowFishException("Incorrect buffer length");
  ...
  if (mode == CBC) //CBC mode, using the Chain
  {
    ...
    for (; n >= 8; n -= 8, in += 8)
    {
      bytes_to_block(in, work);
      work ^= chain;
      encrypt_internal(work);
      chain = work;
      block_to_bytes(work, out+=8);
    }
  }
  else if (mode == CFB) //CFB mode, using the Chain
  {
...
// Returns false if n is multiple of 8
void BlowFish::decrypt(const unsigned char* in, unsigned char* out, size_t n, int mode)
{
  // Check the buffer's length - should be > 0 and multiple of 8
  if ((n == 0) || (n%8 != 0))
    throw BlowFishException("Incorrect buffer length");
...
static unsigned char SOAM_BF_KEY_WMI[] = {
  0x9b, 0xb9, 0x5c, 0xca, 0xc6, 0x62, 0xe1, 0x3e,
  0x19, 0x51, 0xca, 0xd8, 0x63, 0xd5, 0xb7, 0xcd,
  0xbb, 0x4b, 0xa1, 0xec, 0x21, 0x2b, 0xf0, 0x45
};
...
int main(int argc, char const* argv[])
{
  ...
  char const *str = "kamin-gunhee-yielding";
  unsigned char encrypted[sz] = { 0 };

  BlowFish bf(SOAM_BF_KEY_WMI, sizeof(SOAM_BF_KEY_WMI));

  cout << "Original: " << str << endl;
  bf.encrypt((unsigned char*)str, encrypted, sz, BlowFish::CBC);
  cout << "encrypted: " << encrypted << endl;

  unsigned char decrypted[sz] = { 0 };
  bf.decrypt(encrypted, decrypted, sz, BlowFish::CBC);
