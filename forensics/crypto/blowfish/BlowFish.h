...
//     a key-expansion part and a data-ancryption part. 
//  Key expansion converts a variable key of at least 1 and at most 56 bytes into 
//  several subkey arrays totaling 4168 bytes. Blowfish has 16 rounds. 
//  Each round consists of a key-dependent permutation, and a key and 
//  data-dependent substitution. All operations are XORs and additions 
//  on 32-bit words.
...
//  The P-array consists of 18 32-bit subkeys: P0, P1,...,P17.  
...
//  are used:
//
//  In ECB mode if the same block is encrypted twice with the same key, 
//  the resulting ciphertext blocks are the same.
//
//  In CBC Mode a ciphertext block is obtained by first xoring the
//  plaintext block with the previous ciphertext block, and encrypting 
//  the resulting value.
//
...
//
//  The previous ciphertext block is usually stored in an 
//  Initialization Vector (IV).  An Initialization Vector of zero is commonly 
//  used for the first block, though other arrangements are also in use.
...
//
////////////////////////////////////////////////////////////////////////////////
class BlowFishException
{
public:
  enum { MSG_SIZE = 1024 };

public: 
  BlowFishException(char const* str)
  {
    strncpy(m_msg, str, MSG_SIZE);
  }

  char const* what() { return m_msg; }

private:
  char m_msg[MSG_SIZE + 1];
};
...
struct sblock
{
	sblock (unsigned l=0, unsigned r=0) 
    : m_uil(l), m_uir(r) {}
  ...
  { 
    m_uil ^= b.m_uil; m_uir ^= b.m_uir; 

    return *this; 
  }

	unsigned m_uil, m_uir;
};
...
public:
	...
	void encrypt_inplace(unsigned char* buf, size_t n, int iMode=ECB);
	void decrypt_inplace(unsigned char* buf, size_t n, int iMode=ECB);
...
private:
	void encrypt_internal(sblock&);
	void decrypt_internal(sblock&);
