...
constexpr auto operator|(std::expected<T, E>&& ex, F&& f)
    noexcept(noexcept(std::forward<F>(f)(std::move(ex))))
...
struct AndThenF {
    ...
    constexpr auto operator()(std::expected<T, E>&& ex) const
        noexcept(noexcept(std::move(ex).and_then(f)))
...
struct TransformF {
    ...
    template <class T, class E>
    constexpr auto operator()(std::expected<T, E>&& ex) const
        noexcept(noexcept(std::move(ex).transform(f)))
...
struct TeeValueF {
    ...
    template <class T, class E>
    constexpr auto operator()(std::expected<T, E>&& ex) const
        noexcept(noexcept(fv(*ex)) && std::is_nothrow_move_constructible_v<std::expected<T,E>>)
        -> std::expected<T, E> {
        if (ex) fv(*ex);
        return std::move(ex);
    }
...
struct TeeErrorF {
    ...
    template <class T, class E>
    constexpr auto operator()(std::expected<T, E>&& ex) const
        noexcept(noexcept(fe(ex.error())) && std::is_nothrow_move_constructible_v<std::expected<T,E>>)
        -> std::expected<T, E> {
        if (!ex) fe(ex.error());
        return std::move(ex);
    }
...
struct TeeBothF {
    ...
    constexpr auto operator()(std::expected<T, E>&& ex) const
        noexcept(
            (noexcept(fv(*ex)) || noexcept(fe(ex.error()))) &&
            std::is_nothrow_move_constructible_v<std::expected<T,E>>
        )
        -> std::expected<T, E> {
        if (ex) fv(*ex);
        else    fe(ex.error());
...
struct TapF {
    ...
    constexpr auto operator()(std::expected<T,E>&& ex) const
        ...
        -> std::expected<T,E> {
        if (ex) fv(*ex); else fe(ex.error());
        return std::move(ex);
    }
};
template <class FV, class FE>
constexpr auto tap_f(FV fv, FE fe) { return TapF<FV,FE>{std::move(fv), std::move(fe)}; }
