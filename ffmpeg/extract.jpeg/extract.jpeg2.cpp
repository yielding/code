...
// ----- Resize frame -----
AVFramePtr resize_frame(AVFrame* src, AVCodecContext* ctx, int dst_w, int dst_h, AVPixelFormat dst_fmt) 
{
    AVFramePtr dst(av_frame_alloc(), AVFrameDeleter);
    dst->format = dst_fmt;
    dst->width = dst_w;
    dst->height = dst_h;

    if (av_frame_get_buffer(dst.get(), 32) < 0) return nullptr;
...
int save_frame_as_jpeg(AVCodecContext* src_ctx, AVFrame* frame, int frame_no, int out_w, int out_h) 
{
    ...
    AVCodecContextPtr jpeg_ctx(avcodec_alloc_context3(jpeg_codec), AVCodecContextDeleter);
    // jpeg_ctx->pix_fmt = src_ctx->pix_fmt;
    ...
    {
        cerr << "Failed to open JPEG codec" << endl;
        return -1;
    }

    AVFramePtr resized = resize_frame(frame, src_ctx, out_w, out_h, jpeg_ctx->pix_fmt);
    if (!resized) return -1;

    if (avcodec_send_frame(jpeg_ctx.get(), resized.get()) < 0) return -1;

    AVPacketPtr pkt(av_packet_alloc(), AVPacketDeleter);
    if (avcodec_receive_packet(jpeg_ctx.get(), pkt.get()) < 0) return -1;
...
-> tuple<int, AVFormatContextPtr, AVCodecContextPtr> 
{
    AVFormatContext* raw_fmt = nullptr;
    if (avformat_open_input(&raw_fmt, filename, nullptr, nullptr) < 0) 
        return make_tuple(-1, nullptr, nullptr);

    AVFormatContextPtr fmt_ctx(raw_fmt, AVFormatContextDeleter);
    ...
    AVCodecContextPtr dec_ctx(raw_ctx, AVCodecContextDeleter);
