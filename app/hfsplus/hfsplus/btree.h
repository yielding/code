  ...
  : m_file(file)
{
    ...
    if (btnode.kind != kBTHeaderNode)
        throw std::runtime_error("Header node type should be kbBTHeaderNode");
    ...
    for (uint32_t i=1; i<m_blocks_for_node; i++)
    {
...
{
    ByteBuffer node;
    for (uint32_t i=0; i<m_blocks_for_node; i++)
    {
  ...
  -> Record
{
    ...
    if (node.index())
    {
        for (size_t i=0; i<node.recs.size(); i++)
        {
