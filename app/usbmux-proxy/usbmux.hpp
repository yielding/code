#ifndef USBMUX_H_1S8UR7HN
#define USBMUX_H_1S8UR7HN
...
struct usbmux_response
{
  ...
  uint32_t result;        // result code of last request
};
...
{
  usbmux_header header;
  uint32_t  device_id;    // Device ID - Is always 0x19
  uint16_t  dst_port;     // TCP port number
  uint16_t  reserved;
};
...
{
public:
  proxy_session(boost::asio::io_service& io_service, uint16_t remote_port);
...
public:
  ...
  void handle_receive_hello_response(const boost::system::error_code& error, size_t bytes_transferred);
  void receive_device_id();
  void handle_receive_device_id(const boost::system::error_code& error, size_t bytes_transferred);
  void send_connect();
  void handle_send_connect(const boost::system::error_code& error);
  void receive_connect_response();
  void handle_receive_connect_response(const boost::system::error_code& error, size_t bytes_transferred);

  void read_from_client();
  void handle_read_from_client(const boost::system::error_code& error, size_t bytes_transferred);
  void write_to_usbmux();
  void handle_write_to_usbmux(const boost::system::error_code& error, size_t bytes_transferred);

  void read_from_usbmux();
  void handle_read_from_usbmux(const boost::system::error_code& error, size_t bytes_transferred);
...
{
public:
  proxy(boost::asio::io_service& io_service, const boost::asio::ip::tcp::endpoint& endpoint, uint16_t remote_port);
