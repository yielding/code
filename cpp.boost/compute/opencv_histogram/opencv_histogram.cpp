...
inline void 
showHistogramWindow(cv::Mat &b_hist, cv::Mat &g_hist, cv::Mat &r_hist,
...
void calculateHistogramUsingCL(cv::Mat src, compute::command_queue &queue)
{
  ...
  auto gpu_src =
    compute::opencv_create_image2d_with_mat(
        src, compute::image2d::read_only, queue);
  ...
  size_t origin[2] = { 0, 0 };
  size_t region[2] = { gpu_src.width(), gpu_src.height() };
...
int main(int argc, char** argv)
{
  ...
  if (vm.count("image"))
  {
    ...
    src = cv::imread(vm["image"].as<std::string>(), CV_LOAD_IMAGE_COLOR);
    if (!src.data)
    {
