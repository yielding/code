...
int main()
{
  ...
  cout << "device: " << gpu.name() << endl;

  // initialize option data on host
  vector<float> stock_price_data(N);
  vector<float> option_strike_data(N);
  vector<float> option_years_data(N);
  ...
  cout << "option 0 call price: " << call0 << endl;
  cout << "option 0 put price: "  << put0  << endl;
  ...
  // check option prices
  if (abs(call0 - expected_call0) > 1e-4 || abs(put0 - expected_put0) > 1e-4) {
    cerr << "error: option prices are wrong" << endl;
    return -1;
  }
