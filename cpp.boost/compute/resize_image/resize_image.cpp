...
const char source[] = BOOST_COMPUTE_STRINGIZE_SOURCE(
  __kernel void
  resize_image(__read_only image2d_t input, const sampler_t sampler, __write_only image2d_t output)
  {
...
void ImageWidget::initializeGL()
{
  ...
  auto format = compute::qt_qimage_format_to_image_format(qt_image_.format());
  ...
  sampler_ = compute::image_sampler(context_, false, CL_ADDRESS_NONE, CL_FILTER_LINEAR);
...
void ImageWidget::resizeGL(int width, int height)
{
  ...
  cl_texture_ = compute::opengl_texture(context_, GL_TEXTURE_2D, 0, gl_texture_, CL_MEM_WRITE_ONLY);
}
