...
{
  array_source src(bible.data(), bible.data() + bible.size());
  cout << "size of original text: " << bible.size() << endl;

  string temp;
  string dest;
  iostreams::copy(src, compose(zlib_compressor(), non_blocking_sink(temp, 10)));
  cout << "size of compressted text: " << temp.size() << endl;

  iostreams::copy(
    compose(zlib_decompressor(), non_blocking_source(temp, 10)),
    iostreams::back_inserter(dest)
  );

  cout << "size of decompressted text: " << dest.size() << endl;
